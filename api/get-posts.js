...
// Cache em memória: reusa o resultado por 3 minutos para economizar chamadas

let cache = null;
let cacheTime = 0;
const CACHE_TTL = 3 * 60 * 1000; // 3 minutos em ms
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    function getCanvaUrl(props) {
      const urlProp = props['Link'] || props['URL'] ||
    ...
    function isPinned(props) {
      ...
      if (fixarProp.type === 'multi_select')
        return (fixarProp.multi_select || []).some(o => VALS.includes(o.name.toLowerCase()));
      return false;
    }
      ...
      if (tipo === 'Perfil') {
        ...
        destaques.push({
          ...
          ordem: getNum(props['Ordem'] || props['Order']) !== null
            ? getNum(props['Ordem'] || props['Order']) : naturalIndex * 1000,
          imageUrl, canvaUrl
        });
      } else {
        const dateProp =
          props['Data'] || props['Data de publicação'] || props['Publish Date'] || props['Date'] ||
    ...
    const result = { perfil, destaques, posts };
